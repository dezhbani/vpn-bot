...
const EditProfile = () => {
    ...
    return (
        <>
            ...
            <div className="w-full py-5 bg-white rounded-lg">
                <div className="max-sm:flex-col text-lg sm:text-xl flex h-fit font-bold w-full sm:justify-between">
                    <h1 className='flex max-sm:justify-start items-center max-sm:w-full'>
                        <span className='mx-2 inline-block bg-main-blue rounded-full w-2.5 h-2.5' />
                        اطلاعات کاربر
                    </h1>
                    <div onClick={openEditMode} className='flex h-10 max-sm:justify-start max-sm:w-full font-medium'>
                        <img className='sm:mx-3 h-6 sm:h-8 cursor-pointer transition duration-200 ease-in-out transform hover:scale-110' src={EditIcon} alt='EditIcon' />
                        <span className={`${!editMode && 'hidden'} transition-opacity duration-300`}>ویرایش</span>
                    </div>
                </div>
                <div className='flex flex-wrap text-xl w-full justify-center'>
                    <div className='flex flex-col items-center max-sm:max-w-4/5 '>
                        <div className='flex flex-col max-sm:my-10 sm:m-10'>
                            <label className='text-xs text-gray-500'>نام و نام خانوادگی فارسی</label>
                            <input className={`${editMode ? 'border-b-2 border-main-blue focus:border-red-500 transform transition duration-500' : 'opacity-50 border-b-2 border-silver'} outline-none py-1 mt-1 px-2 w-full dir-rtl`} value={data.full_name || ''} name='full_name' onChange={change} readOnly={!editMode} />
                        </div>
                        {
                            editMobile ?
                                    ...
                                    <div className='flex dir-ltr'>
                                        {
                                            [1, 2, 3, 4, 5].map((num, index) => <input key={index} onKeyUp={handleKeyPress(index)} ref={(ref) =>
                                                ...
                                                className="w-8 h-8 flex justify-center rounded text-center my-1 mx-2 bg-[#f9fbff] text-lg border-[2px] border-solid transition-all delay-200 ease-in focus:border-main-blue outline-none"
                                                minLength="1"
                                                maxLength="1"
                                                onChange={changeOTP}
                                                name={num}
                                ...
                                : <div className='flex flex-col max-sm:my-10 sm:m-10'>
                                    ...
                                    <input className={`${editMode ? 'border-b-2 border-main-blue focus:border-red-500 transform transition duration-500' : 'opacity-50 border-b-2 border-silver'} outline-none py-1 mt-1 px-2 w-full dir-ltr`} value={data.mobile || ''} name='mobile' onChange={change} readOnly={!editMode} />
                                </div>
                        }
                    </div>
                    <div className='flex flex-col'>
                        <div className='flex flex-col max-sm:my-10 sm:m-10'>
                            ...
                            <input className={`${editMode ? 'border-b-2 border-main-blue focus:border-red-500 transform transition duration-500' : 'opacity-50 border-b-2 border-silver'} outline-none py-1 mt-1 px-2 w-full dir-ltr`} value={data.first_name || ''} name='first_name' onChange={change} readOnly={!editMode} />
                        </div>
                        <div className='flex flex-col max-sm:my-10 sm:m-10'>
                            <label className='text-xs text-gray-500'>نام خانوادگی لاتین</label>
                            <input className={`${editMode ? 'border-b-2 border-main-blue focus:border-red-500 transform transition duration-500' : 'opacity-50 border-b-2 border-silver'} outline-none py-1 mt-1 px-2 w-full dir-ltr`} value={data.last_name || ''} name='last_name' onChange={change} readOnly={!editMode} />
                        </div>
                    </div>
                </div>
                {
                    editMode &&
                    <div className='w-full flex justify-center mt-5 text-base sm:text-xl'>
                        <button className='w-36 bg-gray-200 px-6 py-1 mx-2 sm:mx-5 rounded-lg' onClick={cancelEdit}>انصراف</button>
                        <button className='w-36 bg-main-blue text-white px-6 py-1 mx-2 sm:mx-5 rounded-lg' onClick={confirmEdit}>ثبت</button>
                    </div>
                }
            </div>
        </>
...
//     <div className='flex flex-col items-center max-sm:max-w-4/5 '>
//         <div className='flex flex-col max-sm:my-10 max-w-48 sm:m-10'>
//             <label className='text-xs text-gray-500'>نام و نام خانوادگی فارسی</label>
